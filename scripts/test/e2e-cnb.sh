#!/usr/bin/env bash
# e2e-cnb.sh — the executable acceptance test for the CNB pipeline (cv-cnb).
#
# Runs the real cv-cnb Pipeline against the live OrbStack cluster with the
...
# (deploy BY @sha256 from zot — the opposite of the crane/apko e2e.sh), and
# cleans up even if an assertion fails.
#
#   scripts/test/e2e-cnb.sh          happy path
#   scripts/test/e2e-cnb.sh --keep   leave the namespace + artifacts + zot repo
#
...
capture() {
	log_kv step=e2e-cnb action=capture-evidence dir="$ART"
...
cleanup() {
	...
	[ -n "${PRN:-}" ] && capture
	if [ "$KEEP" -eq 1 ]; then
		log_kv step=e2e-cnb action=kept ns="$NS" repo="$APP_REPO" artifacts="$ART"
	else
...
log_kv step=e2e-cnb action=setup ns="$NS" fixture="$FIXTURE_SHA" repo="$APP_REPO"
kubectl create ns "$NS" >/dev/null
sed "s/namespace: cv-pipeline/namespace: $NS/" manifests/rbac.yaml | kubectl apply -f - >/dev/null
kubectl -n "$NS" apply -f tasks/buildpacks.yaml -f pipeline/pipeline-cnb.yaml >/dev/null

PRN="$(kubectl -n "$NS" create -o name -f - <<EOF | sed 's#.*/##'
apiVersion: tekton.dev/v1
kind: PipelineRun
metadata: { generateName: e2e-cnb- }
spec:
  pipelineRef: { name: cv-cnb }
  taskRunTemplate:
...
log_kv step=e2e-cnb action=pipelinerun-started run="$PRN"
...
log_kv step=e2e-cnb action=pipelinerun-succeeded run="$PRN"

echo
echo "e2e-cnb assertions:"
...
if [ "$FAILURES" -eq 0 ]; then
	log_kv step=e2e-cnb result=PASS run="$PRN"
else
	die "e2e-cnb: $FAILURES assertion(s) failed — evidence in $ART"
fi
