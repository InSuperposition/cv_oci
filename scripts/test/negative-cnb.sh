#!/usr/bin/env bash
# negative-cnb.sh — prove the CNB pipeline (cv-cnb) FAILS where it should.
#
...
# ---- scenario 1: bad ref ------------------------------------------------
log_kv step=negative-cnb scenario=bad-ref
...
log_kv step=negative-cnb scenario=pre-healthz sha="$PRE_HEALTHZ_SHA"
...
if [ "$FAILURES" -eq 0 ]; then
	log_kv step=negative-cnb result=PASS
else
	die "negative-cnb: $FAILURES assertion(s) failed"
fi
